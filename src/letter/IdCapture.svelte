<script>
  ...
  async function grabStill() {
    ...
    let captureDevice = new ImageCapture(videoTrack);
    if (captureDevice) {
      try {
        const blob = await captureDevice.takePhoto();
        imageObjectUrl = window.URL.createObjectURL(blob);
        imageElement.src = imageObjectUrl;

        if (cropper) {
          cropper.replace(imageObjectUrl, true)
        } else {
          const Cropper = await getCropperModule()
      ...
      console.error('no capture device')
    }
  }
